...
import type { DeviceListResult, CommandOrError, ScreenshotOrError, StreamConfig, IpcTransport } from "../types";
...
export class IpcClient {
	...
	androidTap(params: { deviceId?: string; x: number; y: number }): Promise<CommandOrError> {
		return this.transport.invoke(CH.ANDROID_TAP, params);
	}
	androidSwipe(params: { deviceId?: string; x1: number; y1: number; x2: number; y2: number; duration?: number }): Promise<CommandOrError> {
		return this.transport.invoke(CH.ANDROID_SWIPE, params);
	}
	androidText(params: { deviceId?: string; text: string }): Promise<CommandOrError> {
		return this.transport.invoke(CH.ANDROID_TEXT, params);
	}
	androidLiveStart(deviceId?: string): Promise<StreamConfig & { ok: true } | { ok: false; error: string }> {
	...
	onAndroidLiveChunk(cb: (chunk: { data: ArrayBuffer; timestamp: number }) => void): () => void {
	...
	openEmulator(): Promise<{ ok: true; avd: string } | { ok: false; error: string }> {
	...
	iosTap(params: { udid: string; x: number; y: number }): Promise<CommandOrError> {
		return this.transport.invoke(CH.IOS_TAP, params);
	}
	iosSwipe(params: { udid: string; x1: number; y1: number; x2: number; y2: number; duration?: number }): Promise<CommandOrError> {
		return this.transport.invoke(CH.IOS_SWIPE, params);
	}
	iosLiveStart(udid: string): Promise<StreamConfig & { ok: true } | { ok: false; error: string }> {
		return this.transport.invoke(CH.IOS_LIVE_START, udid);
	}
	iosLiveStop(): Promise<CommandOrError> {
		return this.transport.invoke(CH.IOS_LIVE_STOP);
	}
	onIosLiveChunk(cb: (chunk: { data: ArrayBuffer; timestamp: number }) => void): () => void {
