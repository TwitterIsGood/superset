...
export async function tap(deviceId: string | undefined, x: number, y: number) {
	const result = await run("adb", deviceArgs(deviceId, "shell", "input", "tap", String(x), String(y)), { timeout: 8_000 });
	return result.ok ? { ok: true as const } : { ok: false as const, error: result.stderr };
}
...
export async function swipe(
	...
	const result = await run("adb", deviceArgs(deviceId, "shell", "input", "swipe", String(x1), String(y1), String(x2), String(y2), String(dur)), { timeout: 8_000 });
	return result.ok ? { ok: true as const } : { ok: false as const, error: result.stderr };
}

export async function text(deviceId: string | undefined, value: string) {
	const escaped = value.replace(/%/g, "%s").replace(/\s/g, "%s");
	const result = await run("adb", deviceArgs(deviceId, "shell", "input", "text", escaped), { timeout: 8_000 });
	return result.ok ? { ok: true as const } : { ok: false as const, error: result.stderr };
}
