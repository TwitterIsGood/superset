...
export function V2WorkspacePageContent({
	...
	return <V2WorkspaceContent search={search} />;
}

function V2WorkspaceContent({ search }: V2WorkspacePageContentProps) {
	const {
		terminalId,
		chatSessionId,
		focusRequestId,
			...
			for (const tab of state.tabs) {
				for (const pane of Object.values(tab.panes)) {
					if (pane.kind === "terminal") {
						const { terminalId } = pane.data as TerminalPaneData;
						void releaseTerminalRuntimeLazy(terminalId, pane.id);
