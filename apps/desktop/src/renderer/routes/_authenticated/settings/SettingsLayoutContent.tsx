...
function getSectionFromPath(pathname: string): SettingsSection | null {
	...
	if (pathname.includes("/settings/tools-and-skills")) return "toolsAndSkills";
...
function getPathFromSection(section: SettingsSection): string {
	switch (section) {
		...
		case "git":
			return "/settings/git";
		case "toolsAndSkills":
			return "/settings/tools-and-skills";
		case "terminal":
...
export function SettingsLayoutContent() {
	...
	const usesInnerSidebar =
		...
		location.pathname.startsWith("/settings/tools-and-skills");
