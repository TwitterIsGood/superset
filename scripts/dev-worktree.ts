	...
	try {
		...
		if (options.base !== undefined) {
			...
			for (const [allocatedPath, allocatedValue] of Object.entries(
				...
				if (allocatedPath !== key && allocatedBase === options.base) {
					throw new Error(
						`Port base ${options.base} is already allocated to ${allocatedPath}`,
					);
				}
			}
		...
		if (existing !== undefined && isPortBaseSafe(existing, range)) {
			return existing;
		}
		...
		let candidate = start;
		while (used.has(candidate) || !isPortBaseSafe(candidate, range)) {
			candidate += range;
		}
...
export function buildWorktreeDevPlan(input: {
	...
	const dataPorts = buildPortSet(dataPortBase);
	const workspaceName = basename(input.worktree.currentRoot);
	...
	const env: Record<string, string> = {
		...
		NEXT_PUBLIC_ELECTRIC_URL: urls.caddyElectric,
		NEXT_PUBLIC_ELECTRIC_PROXY_URL: urls.caddyElectric,
	};
...
async function ensureEnvFile(root: string) {
...
function printPlan(plan: WorktreeDevPlan) {
	...
	console.log(
		`  Electric: ${plan.urls.caddyElectric} -> ${plan.urls.electricRaw}`,
	);
...
async function main() {
	...
	console.log("Starting desktop dev graph: bun run dev:desktop:caddy");
	const child = Bun.spawn(["bun", "run", "dev:desktop:caddy"], {
		cwd: plan.currentRoot,
